...
- official BLS CPI-W all-items, motor-fuel, gasoline, other-motor-fuel, gasoline-price, and diesel-price series;
...
- a 3,721-cell gasoline/diesel shock grid for sensitivity analysis.

The fuel shocks are scenario inputs, not causal estimates. BLS defines “other motor fuels” as including automotive diesel and alternative motor fuels, so the diesel result is a proxy for that CPI-W component rather than a diesel-only CPI index. The model does not claim that fuel prices alone determine the COLA. September 2026 is unavailable in the current snapshot, so the current outputs are provisional and should be refreshed after the October BLS release.
